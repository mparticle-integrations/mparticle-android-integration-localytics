...
class LocalyticsKit : KitIntegration(), KitIntegration.EventListener, CommerceListener,
    AttributeListener, PushListener, CallToActionListener {
                ...
                for (i in 0 until customDimensionJson.length()) {
                    ...
                    if (dimension.getString("maptype") == "UserAttributeClass.Name") {
                        ...
                        if (key.equals(attributeName, true)) {
                            return dimension.getString("value").substring("Dimension ".length)
                                .toInt()
                        }
                    }
                }
        ...
        messageList.add(
            ReportingMessage.fromEvent(
                ...
                MPEvent.Builder(eventName, MParticle.EventType.Transaction)
                    .customAttributes(contextInfo).build()
            )
        )
        return messageList
    }
    ...
    override fun logEvent(event: CommerceEvent): List<ReportingMessage> {
        ...
        if (!KitUtils.isEmpty(event.productAction) && event.productAction.equals(
                Product.PURCHASE,
                true
            ) ||
    ...
    override fun willHandlePushMessage(intent: Intent): Boolean {
        ...
        return (intent.extras?.containsKey("ll") == true ||
                intent.extras?.containsKey("localyticsUninstallTrackingPush") == true) &&
                MPUtility.isFirebaseAvailable()
    }

    override fun onPushMessageReceived(context: Context, extras: Intent) {
        val service: Intent
    ...
    override fun localyticsShouldDeeplink(s: String, campaign: Campaign): Boolean = false

    override fun localyticsDidOptOut(b: Boolean, campaign: Campaign) {}

    override fun localyticsDidPrivacyOptOut(optedOut: Boolean, campaign: Campaign) {
